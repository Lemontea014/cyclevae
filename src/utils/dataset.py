...
class FeatureDatasetSingleVAE(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # デバッグ情報: 処理対象のファイルを表示
            #print(f"Processing index: {idx}, Source: {featfile_src}, Target: {featfile_src_trg}")

            # HDF5データの読み込み
            h_src = read_hdf5(featfile_src, "/feat_org_lf0")
            if h_src is None:
                raise ValueError(f"h_src is None for file {featfile_src}")
            ...
            if os.path.basename(os.path.dirname(featfile_src)) == self.spk_src:
                '''
                フレーム分、話者コードが設定される。
                つまり、1発話という単位ではなく、フレーム単位で発話者の識別ができる。
                例えば、1バッチで取得した変換元の発話者が、src話者の場合(src1~40)
                [1,0
                 1,0
                 1,0
                 1,0
            ...
            if h_src_trg is None:
                raise ValueError(f"h_src_trg is None for file {featfile_src_trg}")
            ...
            # デバッグ出力
            # print(f"featfile_src: {featfile_src}")
            # print(f"h_src shape: {h_src.shape}")
            # print(f"flen_src: {flen_src}")
            # print(f"src_code: {src_code}")
            # print(f"trg_code: {trg_code}")
            ...
            return {
                'h_src': h_src,
                'h_src_size': h_src.shape,
                'flen_src': flen_src,
                'src_code': src_code,
                'src_code_size': src_code.shape,
                'trg_code': trg_code,
                'trg_code_size': trg_code.shape,
                'cv_src': cv_src,
                'cv_src_size': cv_src.shape,
                'spcidx_src': spcidx_src,
                'spcidx_src_size': spcidx_src.shape,
                'flen_spc_src': flen_spc_src,
                'h_src_trg': h_src_trg,
                'h_src_trg_size': h_src_trg.shape,
                'flen_src_trg': flen_src_trg,
                'spcidx_src_trg': spcidx_src_trg,
                'spcidx_src_trg_size': spcidx_src_trg.shape,
                'flen_spc_src_trg': flen_spc_src_trg,
                'featfile_src': featfile_src,
                'featfile_src_trg': featfile_src_trg
            }
        
        except Exception as e:
            print(f"Error occurred in __getitem__ at index {idx}: {e}")
            raise  # エラーを再度スローして処理を中断
