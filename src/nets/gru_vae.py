...
class GRU_RNN(nn.Module):
    ...
    def __init__(self, in_dim=39, out_dim=35, hidden_units=1024, hidden_layers=1, kernel_size=3, dilation_size=2, do_prob=0, scale_in_flag=True, scale_out_flag=True, scale_in_out_flag=False):
        ...
        if self.scale_in_flag:
            #Zスコア標準化を行う層。各ニューロンの重みとbiasが、各次元に対する標準化を行う。更に、この層は勾配更新がない
            self.scale_in = nn.Conv1d(self.in_dim, self.in_dim, 1)

        # Dilated two-sides-causality convolution layers: +- 13 frames with kernel_size = 3 and dilation_size = 2
        #拡張畳み込み層の定義 TwoSidedDilConv1dは2層構成。1層目の拡張数は1、2層目の拡張数は3になる。
        self.conv = TwoSidedDilConv1d(in_dim=self.in_dim, kernel_size=self.kernel_size, layers=self.dilation_size)
        #rec_field（受容野)の定義は、self.kernel_size**self.layers
        #今回のオブジェクトは、k=3  layer=2 なので、self.receptive_field=9
        self.receptive_field = self.conv.rec_field
        #拡張した特徴量の最終出力 デフォルトは　486+潜在特徴量の数×2= 518
    ...
    def get_config(self):
        ...
        return {
            ...
            "scale_in_flag": self.scale_in_flag,
            "scale_out_flag": self.scale_out_flag,
            "scale_in_out_flag": self.scale_in_out_flag,
        }
