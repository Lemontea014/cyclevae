        ...
        with torch.cuda.device(gpu):
            ...
            for feat_file, wav_file, wav_trg_file in zip(feat_list, wav_list, wav_trg_list):
                ...
                with torch.no_grad():
                    ...
                    logging.info(f"cvmcepは、C-contignousか？ : {cvmcep.is_contiguous()}")
                    if not cvmcep.is_contiguous():
                        cvmcep = cvmcep.contiguous()
                    ...
                    if not cvmcep_src.is_contiguous():
                        cvmcep_src = cvmcep_src.contiguous()
                    ...
                    if not cvmcep_trg.is_contiguous():
                        cvmcep_trg = cvmcep_trg.contiguous()
                ...
                '''
                パワー比率を修正
                mcepの0次係数に、対数パワーの比率を加えて、変換前と後のエネルギーをあわせている。
                '''
                ...
                '''
                Merlinを使ったフォルマント強調のポストフィルタ

                '''
                logging.info("変換後のメルケプストラムに、フォルマント強調のポストフィルタをかける。")
                cvmcep = merlin_post_filter(cvmcep, alpha=args.mcep_alpha)
                cvmcep_src = merlin_post_filter(cvmcep_src, alpha=args.mcep_alpha)
                cvmcep_trg = merlin_post_filter(cvmcep_trg, alpha=args.mcep_alpha)


                logging.info("cvmcep_gv")
                datamean = np.mean(cvmcep[:,1:], axis=0)
                #変換メルケプストラムに、GVポストフィルタ[2012 Toda]の実装
                ...
                #ポストフィルタをかけた後にも、エネルギー正規化を更に行う。ここいるのかな？
