...
#mcep、スペクトログラムのフレームごとのパワー、閾値を受け取る。
def extfrm(data, npow, power_threshold=-20):
    T = data.shape[0]
    if T != len(npow):
        raise("Length of two vectors is different.")
    #-20よりも、大きいnpowを、有声音区間とする。
...
def main():
    ...
    #verboseはrun.shで明示的に渡されないため、デフォルトの1となる。
    parser.add_argument(
        "--verbose", default=1,
    ...
    # read list
    # args.waveformsはscpファイル（wavファイルのパスリスト）
    # ファイルなので、elseが読まれるはず。
    if os.path.isdir(args.waveforms):
        
        logging.info("args.waveforms はディレクトリです。")
        file_list = sorted(find_files(args.waveforms, "*.wav"))
        
    else:
        logging.info("args.waveforms はファイルです。")
        file_list = read_txt(args.waveforms)
        
        size = len(file_list)
        logging.info(f"file_list のサイズ: {size}")
        logging.info(f"file_list の内容: {file_list}")
    ...
    def feature_extract(wav_list, arr):
        ...
        logging.info(f"タスク {wav_list}を開始")
        ...
        for wav_name in wav_list:
            ...
            n_sample += x.shape[0]
            logging.info(wav_name+" 音声のサンプル数は:"+str(x.shape[0])+" wav_lists内の音声サンプルの総計は:"+str(n_sample)+" "+str(count))
            ...
            #音声信号の長さ、指定した範囲のf0、スペクトル包絡、非周期性指標を抽出
            ...
            #logging.info(f"時間: {time_axis_range}")
            #logging.info(f"基本周波数: {f0_range}")
            #logging.info(f"スペクトル包絡: {spc_range}")
            #logging.info(f"非周期性指標: {ap_range}")
            ...
            #有声音部分のメルケプストラムフレーム（mcepscp) と、インデックス(フレーム番号）を返す。
            mcepspc_range, spcidx_range = extfrm(mcep_range, npow_range, power_threshold=args.pow)
            logging.info(wav_name+" mcepscp_rangeの行数"+str(mcepspc_range.shape[0])+" mcepの列数（次元数）："+str(mcepspc_range.shape[1])+" "+str(count))
            
            #f0を列ベクトルから行ベクトルへ変換
            cont_f0_lpf = np.expand_dims(cont_f0_lpf, axis=-1)
            cont_f0_lpf_range = np.expand_dims(cont_f0_lpf_range, axis=-1)
            #有声フラグを列ベクトルから行ベクトルへ変換
            uv = np.expand_dims(uv, axis=-1)
            uv_range = np.expand_dims(uv_range, axis=-1)
            #帯域非周期性指標を列ベクトルから行ベクトルへ変換
            if codeap.ndim == 1:
                logging.info(f"変換前のcodeap: {codeap.shape}")
                codeap = np.expand_dims(codeap, axis=-1)
            if codeap_range.ndim == 1:
                logging.info(f"変換前のcodeap_range: {codeap.shape}")
                codeap_range = np.expand_dims(codeap_range, axis=-1)
                
            logging.info(f"uv: {uv.shape}")
            logging.info(f"cont_f0_lpf: {cont_f0_lpf.shape}")
            logging.info(f"mcep: {mcep.shape}")
            logging.info(f"codeap: {codeap.shape}")
            
                
            #特徴量の結合（54次元）
            feats = np.concatenate([uv, cont_f0_lpf, mcep, codeap], axis=1)
            logging.info(f"{wav_name} の特徴量: {feats.shape}")
            ...
            #n_frameは、全データの特徴量の時間フレームを加えたもの
        ...
        if (len(wav_list) > 0):
            #wav_listが空でない場合に実行。
            #40つにwav_listを分割しているので、テストデータは最後の5つのリストが空になる。
            #logging.info(str(len(wav_list))+" "+str(n_sample/len(wav_list))+" "+str(n_frame/len(wav_list)))
            logging.info(f"wav_listの内容:{wav_list} 音声データのサンプル点数:{n_sample/len(wav_list)} 特徴量の時間フレーム：　{n_frame/len(wav_list)}")
            
        logging.info("max_frame = %d" % (max_frame))
        logging.info(f"タスク {wav_list}を終了")
        logging.info(f"累積終了タスク: {arr[0]}")

    # divie list
    file_lists = np.array_split(file_list, args.n_jobs)
    # file_listを40個のファイルリストに分割
    
    file_lists = [f_list.tolist() for f_list in file_lists]
    logging.info(f"file_lists の内容: {file_lists}")
    ...
    for f in file_lists:
        ...
        #feature_extract(f,arr)を並列にわたす処理
        p.start()
        processes.append(p)
    ...
    for i, p in enumerate(processes):
        logging.info(f"Process {i}: Alive? {p.is_alive()}, Exit code: {p.exitcode}")
        
    #logging.info(str(arr[0])+" "+str(arr[1])+" "+str(arr[1]/arr[0])+" "+str(arr[2])+" "+str(arr[2]/arr[0]))
    logging.info(f"{args.waveforms} の特徴量抽出が完了しました。")
    logging.info(f"総計wav_list数:{arr[0]} 音声データのサンプル点数平均:{arr[1]/arr[0]} 特徴量の時間フレーム平均：{str(arr[2]/arr[0])}")
