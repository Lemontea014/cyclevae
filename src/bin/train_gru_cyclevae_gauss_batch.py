...
def main():
    ...
    parser.add_argument("--add_epoch", default=300,
                        type=int, help="number of training epochs")
    # other setting
    ...
    model_decoder.train()
    '''
    torch.nn.Parameter(torch.diag(1.0/std_jnt.data).unsqueeze(2))は、
    data型でパラメータ参照をおこなっているので、この層(scale_in)は明示的に勾配更新が行われない。
    逆に、動的に管理することはできなくなる。
    '''
    ...
    if args.resume is None:
        epoch_idx = 0
        args.add_epoch=args.epoch_count
    else:
    ...
    #学習ループ
    while epoch_idx < args.add_epoch:
        ...
        if c_idx_src < 0:
            ...
            logging.info("estimated training required time = {0.days:02}:{0.hours:02}:{0.minutes:02}:{0.seconds:02}".format(relativedelta(seconds=int((args.add_epoch - (epoch_idx + 1)) * np.sum(total)))))
            model_encoder.eval()
            model_decoder.eval()
            for param in model_encoder.parameters():
                param.requires_grad = False
