...
# Modified and Improved （enable additional learning and Norm improvement）by Katou , 2025　
...
echo "train_src_trg = $train_src_trg"
...
in_dim=54
echo "入力する音響特徴量の次元数は$in_dim次元"
out_dim=50
echo "出力する音響特徴量の次元数は$out_dim次元"

lat_dim=16
echo "潜在変数の次元数:$lat_dim"
#変換スペクトルの入力を、3回行う。
n_cyc=3
...
#GRUの隠れ層（CNNの最終出力の結合層なので、1層で良いはず）
hidden_layers=1
hidden_units=1024
...
#ミニバッチのサイズ（より詳細な特徴を扱うなら、80より多くしても良い？）
batch_size=80
...
#初期学習が終わって追加学習したい場合はここ変えてね
add_epoch=300

#以前学習したエポック数を入れてね（pre_epoch.pklをロードして追加学習を始めるよ)
pre_epoch=180
...
if [[ "${stage}" == *0* ]]; then
 ...
 ## ソース話者データの分割 ##
    ...
    if [ -e data/${train_src}/wav.scp ]; then
     echo "ファイル data/${train_src}/wav.scp がすでに存在します。削除します。"
     rm data/${train_src}/wav.scp
    fi
    
    if [ -e data/${train_trg_src}/wav.scp ]; then
     echo "ファイル data/${train_trg_src}/wav.scp がすでに存在します。削除します。"
     rm data/${train_trg_src}/wav.scp
    fi
     
    if [ -e data/${eval_src}/wav.scp ]; then
     echo "ファイル data/${eval_src}/wav.scp がすでに存在します。削除します。"
     rm data/${eval_src}/wav.scp
    fi
    
    if [ -e data/${test_src}/wav.scp ]; then
     echo "ファイル data/${test_src}/wav.scp がすでに存在します。削除します。"
     rm data/${test_src}/wav.scp
    fi

    for spk in ${spks_src[@]};do
        #spkは空の変数で、spkに、${spks_src}の要素を1つ代入し、spkを使った処理を下記で行う。これを配列のすべての要素に対して順番に繰り返し処理を行う。
        #今回の場合はspks_src[@]の内容はVCC2SF1である。
        wav_count=$(find wav/${spk} -name "*.wav" | wc -l)
        echo "${spk} の .wav データ数: ${wav_count}"
        
        #先頭45データを訓練データ1とする（1-45を骨伝導音声、46-90を気導音音声としたデータセットを構築します。）
        find wav/${spk} -name "*.wav" | sort | head -n 45 >> data/${train_src}/wav.scp
        #末尾45データを訓練データ2とする（上記とは逆のデータセットを作成します。)
        find wav/${spk} -name "*.wav" | sort | tail -n 45 >> data/${train_trg_src}/wav.scp
        
        #評価データはデータとする。
        wav_count=$(find wav/bone_air_eval/${spk} -name "*.wav" | wc -l)
        echo " ${spk} のテストデータの .wav データ数: ${wav_count}"
        
        find wav/bone_air_eval/${spk} -name "*.wav" | sort >> data/${test_src}/wav.scp
    done
...
## ターゲット話者データの分割　##
    ...
    if [ -e data/${train_trg}/wav.scp ]; then
     echo "ファイル data/${train_trg}/wav.scp がすでに存在します。削除します。"
     rm data/${train_trg}/wav.scp
    fi
     
    if [ -e data/${train_src_trg}/wav.scp ]; then
     echo "ファイル data/${train_src_trg}/wav.scp がすでに存在します。削除します。"
     rm data/${train_src_trg}/wav.scp
    fi
    
    if [ -e data/${eval_trg}/wav.scp ]; then
     echo "ファイル data/${eval_trg}/wav.scp がすでに存在します。削除します。"
     rm data/${eval_trg}/wav.scp
    fi
    
    if [ -e data/${test_trg}/wav.scp ]; then
     echo "ファイル data/${test_trg}/wav.scp がすでに存在します。削除します。"
     rm data/${test_trg}/wav.scp
    fi
    ...
    for spk in ${spks_trg[@]};do
        ...
        find wav/${spk} -name "*.wav" | sort | tail -n 45 >> data/${train_trg}/wav.scp
        find wav/${spk} -name "*.wav" | sort | head -n 45 >> data/${train_src_trg}/wav.scp
        
        wav_count=$(find wav/bone_air_eval/${spk} -name "*.wav" | wc -l)
        echo " ${spk} のテストデータ .wav データ数: ${wav_count}"
        
        find wav/bone_air_eval/${spk} -name "*.wav" | sort >> data/${test_trg}/wav.scp
    done
    
    echo "stage = ${stage} を実行しました"
fi

#evは検証セットなので, データが少量の今回は使用しない. 学習データに大規模コーパスを利用する際にプログラムを追記してください。
...
if [[ "${stage}" == *1* ]];then
    ...
    if true; then
        ...
        for set in ${train_src} ${train_trg_src} ${test_src};do
            ...
            #${train_src} or ${train_trg_src} or ${test_src} の特徴量計算過程を格納するフォルダ
            ...
            for spk in ${spks_src[@]}; do
                ...
                #spkはソース話者
                #src -> trg
                echo " ${set} 内のwavファイル数は $n_wavs"
                
                if [ $n_wavs -gt 0 ]; then
                #今回は　n_wavs = 45 なので、45 > 0 で真
                #data/${set} にあるwav.scpファイルを、future_extract/${set}へコピーする
                    cat data/${set}/wav.scp | grep ${spk} > ${scp}
                    
                    # run.plスクリプトを実行。ジョブ管理、pythonスクリプトの実行、pyファイルの処理logの出力を同時に行ってくれる。
...
if [[ "${stage}" == *2* ]];then
    ...
    if true; then
    #if false; then
        echo "ソース話者の訓練データ: ${train_src}　の統計を計算"
        #train_cmd は、run.plスクリプトを実行するコマンド
        ...
        echo "ターゲット話者の訓練データ: ${train_trg}　の統計を計算"
        ...
        echo "ソース話者とターゲット話者の訓練データ: ${train_src} と ${train_trg}　の結合された統計データを計算"
        #stats_jntデータは、ソース話者の特徴量データフォルダに格納サれる。
    ...
    if true; then
        ...
        ${cuda_cmd} ${expdir}/log/${spk_src_list}-${spk_trg_list}.log \
            train_gru_cyclevae_gauss_batch.py \
                ...
                #--resume ${expdir}/checkpoint-${pre_epoch}.pkl \
