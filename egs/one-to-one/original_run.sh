...
min_idx=98  #sf1-tf1 batch_utt=1 lat_16 cyc=3
...
if [[ "${stage}" == *0* ]]; then
 ...
 ## ソース話者データの分割 ##
    ...
    for spk in ${spks_src[@]};do
        #spkは空の変数で、${spks_src[@]}の要素を1つ代入し、spkを使った処理を下記で行う。これを配列のすべての要素に対して順番に繰り返し処理を行う。
        ...
        #評価データはデータとする。
        wav_count=$(find wav/eval/${spk} -name "*.wav" | wc -l)
    ...
    if true; then
        ...
        for set in ${train_src} ${train_trg_src} ${test_src};do
            ...
            for spk in ${spks_src[@]}; do
                ...
                if [ $n_wavs -gt 0 ]; then
                #今回は　n_wavs = 40 なので、40 > 0 で真
                #data/${set} にあるwav.scpファイルを、future_extract/${set}へコピーする
                    cat data/${set}/wav.scp | grep ${spk} > ${scp}
                    
                    # run.plスクリプトを実行。ジョブ管理、pythonファイルの実行、pyファイルの処理logの出力を同時に行ってくれる。
